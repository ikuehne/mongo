...
AutoGetDbForDbCheck::AutoGetDbForDbCheck(OperationContext* opCtx,
                                         const NamespaceString& nss)
    : _opCtx(opCtx),
      _global(opCtx, MODE_IX, std::numeric_limits<unsigned int>::max()),
      _dbLock(opCtx->lockState(), ResourceId(RESOURCE_DATABASE, nss.db()), MODE_S),
      _db(dbHolder().get(opCtx, nss.ns())) {}

AutoGetCollectionForDbCheck::AutoGetCollectionForDbCheck(OperationContext* opCtx,
                                                         const NamespaceString& nss,
                                                         const OplogEntriesEnum& type)
        : _agd(opCtx, nss),
          _collLock(opCtx->lockState(), nss.ns(), MODE_S) {
    std::string msg;
    if (!_agd.getDb()) {
